...
class PlanningPage : ComponentActivity() {
    ...
    fun PlanningDay(dayPlanning: DayPlanning) {
        Column(
            ...
            // TODO: Determine day text with users "beginningWeekDay"
            Text("Lunes", style = MaterialTheme.typography.titleLarge)
            Spacer(modifier = Modifier.size(12.dp))
            Column(
                verticalArrangement = Arrangement.spacedBy(12.dp),
                horizontalAlignment = Alignment.CenterHorizontally,
            ) {
                PlanningDayRecipe(dayPlanning.lunch)
                PlanningDayRecipe(dayPlanning.dinner)
            }
        }
    }

    @Composable
    fun PlanningDayRecipe(recipe: Recipe) {
        Row {
            Icon(imageVector = Icons.Outlined.NightlightRound, contentDescription = "")
            Spacer(modifier = Modifier.size(12.dp))
            if (recipe == Recipe.EMPTY_RECIPE) {
                QImageContainer(
                    ...
                    // TODO: Show bottom sheet with simplified recipe list
                }
            } else {
                QRecipeItem(
                    recipe = recipe,
